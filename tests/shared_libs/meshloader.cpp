...
bool Mesh::LoadFromFileObj(const char* filename, bool loadMtl)
{
  ...
  //std::cout << "obj filename is " << filename << std::endl;
...
OSPGeometry Mesh::AddToModel(OSPModel model, OSPRenderer renderer, OSPMaterial mtl)
{
    ...
    if (geo.num_faces != 0)
    {
      ...
      if (geo.has_normal) {
	        OSPData ndata = ospNewData(geo.normal.size() / 3, 
									   ...
									   //OSP_DATA_SHARED_BUFFER);
	        ospCommit(ndata);
	        ospSetData(gdata, "vertex.normal", ndata);
	        ospRelease(ndata);
      }
      ...
      if (geo.has_texcoord) {
	        OSPData tdata = ospNewData(geo.texcoord.size() / 2, 
									   ...
									   //OSP_DATA_SHARED_BUFFER);
	        ospCommit(tdata);
	        ospSetData(gdata, "vertex.texcoord", tdata);
	        ospRelease(tdata);
      }
