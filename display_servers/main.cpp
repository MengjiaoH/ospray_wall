...
extern "C" int main(int argc, const char **argv){
    ...
    // std::cout << "display ID is " << displayID.x << " " << displayID.y << " on Rank #" << world.rank << std::endl;
    ...
    if(remote_mode){ 
        ...
        startWallServer(world, displayGroup, dispatchGroup, wallConfig, hasHeadNode, displayNewFrame, glfwWindow, portNum, process_pernode, remote_mode, clientNum);
        if(hasHeadNode && world.rank ==0){
        }else{
            assert(glfwWindow);
            glfwWindow ->run();
